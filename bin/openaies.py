...
class Algo(EvoAlgo):
    ...
    def loadhyperparameters(self):
        ...
        if os.path.isfile(self.fileini):
            ...
            self.symseed = 1
            self.saveeach = 60
            self.joint_variation = 0.1
            options = config.options("ALGO")
            for o in options:
                found = 0
                if o == "joint_variation":
                    self.joint_variation = config.getfloat("ALGO", "joint_variation") 
                    found = 1
                if o == "maxmsteps":
    ...
    def setProcess(self):
        ...
        self.samplefitness2 = zeros(self.batchSize * 2)
    ...
    def evaluate(self):
        ...
        for b in range(self.batchSize):
            for bb in range(2):
                ...
                if (((self.cgen - 1) % 1) == 0):  # - changing for evaluating them at each generation
                    eval_rews, eval_length = self.policy.rollout(self.policy.ntrials, seed=(
                                self.seed + (self.cgen * self.batchSize) + b + 999))
        ...
        if self.bestsol is not None:
            ...
            # TODO: get this code part (until line 214)
    ...
    def run(self):
        ...
        self.policy.env.robot.randInitLow = (-1) * self.joint_variation
        self.policy.env.robot.randInitHigh = self.joint_variation
        print(
            f"####USING  actual range = [{self.policy.env.robot.randInitLow},{self.policy.env.robot.randInitHigh}]")
