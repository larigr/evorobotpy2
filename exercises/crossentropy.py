...
PERCENTILE = (
    70  # Fraction of the best evaluation episode used to train the policy network
)
...
class Net(nn.Module):  # The policy network class
    def __init__(self, obs_size, ninternal, nactions):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(
                ninternal, nactions
            ),  # The output neurons encode the probability if execution of the n different possible actions
        )
...
)  # store the total undiscounted reward and the steps, i.e. the list of observations and actions of an episode
EpisodeStep = namedtuple(
    "EpisodeStep", field_names=["observation", "action"]
)  # list of observations and action of an episode


def rollout(
    env, net, nepisodes
):  # carry on the evaluation episodes and store data (cumulative reward, observations and actions)
    ...
    while True:
        obs_v = torch.FloatTensor(
            [obs]
        )  # convert the observation vector to a tensor vector
        act_probs_v = sm(
            net(obs_v)
        )  # compute the action vector with the policy network and convert the action vector to a probability distribution with softmax
        act_probs = act_probs_v.data.numpy()[
            0
        ]  # convert the action tensor vector in a numpy vector
        action = np.random.choice(
            len(act_probs), p=act_probs
        )  # choses the action randomly by using the action probabilities
        ...
        )  # append the observation and the action to the data
        if is_done:  # at the end of the episode:
            rolldata.append(
                Episode(reward=episode_reward, steps=episode_steps)
            )  # create the data of the episode, i.e. the summed undiscounted reward and the list of observation and actions
...
def filter_rollout(rolldata, percentile):  # filter out the data of the worse episode
    rewards = list(
        map(lambda s: s.reward, rolldata)
    )  # extracts the list of comulative rewards collected in the corresponding episodes
    reward_bound = np.percentile(
        rewards, percentile
    )  # computes the minimum reward which episodes should have to be used for training the policy network
    ...
    for episode in rolldata:
        if (
            episode.reward < reward_bound
        ):  # check whether the reward of the episode exceed the minimal bound
            continue  # data of low performing episodes are discarded
        train_obs.extend(
            map(lambda step: step.observation, episode.steps)
        )  # store the observation for training
        train_act.extend(
            map(lambda step: step.action, episode.steps)
        )  # store the actions for training

    train_obs_v = torch.FloatTensor(
        train_obs
    )  # transform the observation matrix in a tensor
...
if __name__ == "__main__":
    ...
    print(
        "Nepisodes x epoch: %d fraction episode discarded %.2f"
        % (NEPISODES, PERCENTILE)
    )
    env = gym.make("CartPole-v0")  # create the environment
    nsensory = env.observation_space.shape[0]  # extract the number of sensory neurons
    nmotor = (
        env.action_space.n
    )  # extract the number of motor neurons by assuming that the action space of the environment is discrete

    net = Net(nsensory, NINTERNAL, nmotor)  # create the network policy
    print(
        "The policy network has %d sensory, %d internal and %d motor neurons"
        % (nsensory, NINTERNAL, nmotor)
    )
    objective = (
        nn.CrossEntropyLoss()
    )  # the Cross Entropy Loss function combine cross entropy and softmax. It works with raw values (digits) thus avoiding the need to add a softmax layer in the policy network
    optimizer = optim.Adam(
        params=net.parameters(), lr=0.01
    )  # initialize the Adap stochastic optimizer
    print("")
    ...
    for epoch, rolldata in enumerate(rollout(env, net, NEPISODES)):
        ...
        optimizer.step()
        print(
            "epoch %d: loss=%.3f, reward_mean=%.1f, reward_threshould=%.1f"
